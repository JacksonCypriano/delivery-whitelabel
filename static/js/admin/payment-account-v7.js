(function () {
  ...
  var MODAL_ID = 'payment-account-terms-modal-v7';
  ...
  function wrapperFor(field) {
    return field && field.closest('.form-row, td, .fieldBox');
  }
  ...
  function sourceStatusText(row) {
    if (!row) return '';
    var readonly = row.querySelector('.field-status .readonly');
  ...
  function buildCompactControl(row, enabledInput, termsInput) {
    var section = row.querySelector('.payment-online-control-section') || row;
    ...
    var control = {
      ...
      status: status,
      savedStatus: sourceStatusText(row)
    };
  ...
  function initializeAll() {
    document.querySelectorAll('.payment-account-inline .inline-related').forEach(initializeRow);
  }

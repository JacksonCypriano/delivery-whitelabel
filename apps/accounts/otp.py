...
def client_ip(request):
    # Enable only when the trusted reverse proxy overwrites X-Real-IP.
    if settings.OTP_TRUST_PROXY_HEADERS:
...
def active(pending):
    if pending.completed_at or pending.expires_at <= timezone.now():
        raise OTPError('Este cadastro expirou ou já foi concluído. Inicie um novo cadastro.')
    ...
    with transaction.atomic():
        ...
        for key in keys:
            ...
            if len(bucket.events) >= 5:
                raise OTPError('Limite de 5 envios por hora atingido. Tente novamente mais tarde.')
...
def deliver(channel, pending, code):
    message = f'Seu código VemDeDelivery é {code}. Válido por 10 minutos. Não compartilhe este código.'
    if channel == 'email':
        if send_mail('Confirme seu e-mail — VemDeDelivery', message,
                     settings.DEFAULT_FROM_EMAIL, [pending.email]) != 1:
            raise OTPError('Não foi possível enviar o código. Tente reenviar em 60 segundos.')
        return
    if not all([settings.EVOLUTION_API_URL, settings.EVOLUTION_API_KEY, settings.EVOLUTION_INSTANCE]):
        raise OTPError('Envio de WhatsApp indisponível. Tente novamente mais tarde.')
    ...
    if not isinstance(payload, dict) or not isinstance(payload.get('key'), dict) or not payload['key'].get('id'):
        raise OTPError('Não foi possível confirmar o envio. Tente reenviar em 60 segundos.')
...
def send_code(pending_id, ip, channel):
    ...
    with transaction.atomic():
        ...
        if pending.last_sent_at and (now - pending.last_sent_at).total_seconds() < 60:
            raise OTPError('Aguarde 60 segundos entre os envios.')
...
def verify_code(pending_id, channel, code):
    ...
    with transaction.atomic():
        ...
        if not pending.code_hash or not pending.code_expires_at or pending.code_expires_at <= now:
            raise OTPError('Código expirado ou não enviado. Solicite um novo código.')
        if pending.attempts >= 5:
            raise OTPError('Limite de 5 tentativas atingido. Solicite um novo código.')
        pending.attempts += 1
